...
- [x] 实现`demo/data/base_dataset.py` - 数据集基类
  - BaseDataset抽象基类
  - TimeSeriesDataset通用时序数据集
  - SpatialTemporalDataset空间时序数据集
- [x] 实现`demo/data/flow_network_dataset.py` - FlowNetwork专用数据集
  - FlowNetworkDataset类
  - FlowNetworkDataLoader辅助类
- [x] 实现`demo/data/transforms.py` - 数据变换
  - ZScoreScaler归一化
  - MinMaxScaler缩放
...
- [x] 实现`demo/data/selectors.py` - 典型序列选择器
...
- [x] 设计并实现`demo/models/base_model.py` - 模型基类
...
- [x] 实现ARIMA模型 (`demo/models/statistical/arima.py`)
  - ARIMAModel类，支持(p,d,q)参数
  - AutoARIMAModel自动参数选择
- [x] 实现STGNN模型 (`demo/models/deep_learning/`)
...
- [x] 创建实验脚本
  - `demo/experiments/run_arima.py` - ARIMA实验脚本
  - `demo/experiments/run_stgnn.py` - STGNN实验脚本
- [x] 创建工具函数 (`demo/utils/metrics.py`)
...
### 运行ARIMA实验
```bash
uv run --python .venv-torch\Scripts\python python demo/experiments/run_arima.py
```

### 运行STGNN实验
```bash
uv run --python .venv-torch\Scripts\python python demo/experiments/run_stgnn.py
```
